...
TEST_CASE("TestGetQueue") {
  ...
  HERMES_RUN_ADMIN->CreateQueueRoot(hrun::DomainId::GetLocal(), qid,
                                 16, 16, 256,
                                 hshm::bitfield32_t(0));
  HERMES_RUN_CLIENT->GetQueue(qid);

  hshm::Timer t;
  t.Resume();
  size_t ops = (1 << 20);
  for (size_t i = 0; i < ops; ++i) {
    hrun::MultiQueue *queue = HERMES_RUN_CLIENT->GetQueue(qid);
...
TEST_CASE("TestHshmAllocateFree") {
  ...
  for (size_t i = 0; i < reps; ++i) {
    ...
    for (size_t j = 0; j < count; ++j) {
      tasks[j] = HERMES_RUN_CLIENT->NewTaskRoot<hrun::Task>().ptr_;
    }
    for (size_t j = 0; j < count; ++j) {
      HERMES_RUN_CLIENT->DelTask(tasks[j]);
    }
  }
  t.Pause();
  HILOG(kInfo, "Latency: {} MOps", ops / t.GetUsec());
...
TEST_CASE("TestHshmQueueEmplacePop") {
  ...
  auto queue = hipc::make_uptr<hrun::MultiQueue>(
      qid, queue_info);
  hrun::LaneData entry;
  auto task = HERMES_RUN_CLIENT->NewTaskRoot<hrun::Task>();
  entry.p_ = task.shm_;
...
void TestWorkerIterationLatency(u32 num_queues, u32 num_lanes) {
  ...
  HERMES_RUN_ADMIN->RegisterTaskLibRoot(hrun::DomainId::GetLocal(), "small_message");\
