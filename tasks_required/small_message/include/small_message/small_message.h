...
#ifndef HERMES_RUN_small_message_H_
#define HERMES_RUN_small_message_H_
...
class Client : public TaskLibClient {
 ...
 public:
  ...
  LPointer<MdTask> AsyncMd(const TaskNode &task_node,
    ...
    auto task = HERMES_RUN_CLIENT->NewTask<MdTask>(
        task_node, domain_id, id_);
    queue->Emplace(TaskPrio::kLowLatency, 3, task.shm_);
    return task;
  }
  HERMES_RUN_TASK_NODE_ROOT(AsyncMd);
  ...
  void AsyncMdPushConstruct(MdPushTask *task,
                            ...
                            const DomainId &domain_id) {
    HERMES_RUN_CLIENT->ConstructTask<MdPushTask>(
  ...
  int MdPushRoot(const DomainId &domain_id) {
    ...
    int ret = task->ret_[0];
    HERMES_RUN_CLIENT->DelTask(push_task);
    return ret;
  }
  HERMES_RUN_TASK_NODE_PUSH_ROOT(MdPush);

  /** Io task */
  void AsyncIoConstruct(IoTask *task, const TaskNode &task_node,
                        const DomainId &domain_id) {
    HERMES_RUN_CLIENT->ConstructTask<IoTask>(
        task, task_node, domain_id, id_);
  }
  ...
  HERMES_RUN_TASK_NODE_PUSH_ROOT(Io)
};

}  // namespace hrun

#endif  // HERMES_RUN_small_message_H_
