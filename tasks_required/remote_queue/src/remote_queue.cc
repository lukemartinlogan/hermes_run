 ...
 public:
  ...
  void Construct(ConstructTask *task, RunContext &ctx) {
    HILOG(kInfo, "(node {}) Constructing remote queue (task_node={}, task_state={}, method={})",
          HERMES_RUN_CLIENT->node_id_, task->task_node_, task->task_state_, task->method_);
    HERMES_RUN_THALLIUM->RegisterRpc("RpcPushSmall", [this](const tl::request &req,
                                                         TaskStateId state_id,
                                                         u32 method,
                                                         std::string &params) {
      this->RpcPushSmall(req, state_id, method, params);
    });
    HERMES_RUN_THALLIUM->RegisterRpc("RpcPushBulk", [this](const tl::request &req,
                                                        const tl::bulk &bulk,
                                                        TaskStateId state_id,
                                                        u32 method,
                                                        std::string &params,
  ...
  void ClientSmallPush(std::vector<DataTransfer> &xfer, PushTask *task) {
    ...
    for (int replica = 0; replica < task->domain_ids_.size(); ++replica) {
      ...
      HILOG(kDebug, "(SM) Transferring {} bytes of data (task_node={}, task_state={}, method={}, from={}, to={})",
            ...
            task->orig_task_->task_state_,
            task->orig_task_->method_,
            HERMES_RUN_CLIENT->node_id_,
            domain_id.id_);
      std::string ret = HERMES_RUN_THALLIUM->SyncCall<std::string>(domain_id.id_,
                                                                "RpcPushSmall",
                                                                task->exec_->id_,
                                                                task->exec_method_,
                                                                task->params_);
  ...
  void ClientIoPush(std::vector<DataTransfer> &xfer, PushTask *task) {
    ...
    for (int replica = 0; replica < task->domain_ids_.size(); ++replica) {
      ...
      std::string ret = HERMES_RUN_THALLIUM->SyncIoCall<std::string>(domain_id.id_,
                                                                  "RpcPushBulk",
                                                                  io_type,
                                                                  data,
                                                                  data_size,
 ...
 private:
  ...
  void RpcPushBulk(const tl::request &req,
    ...
    if (io_type == IoType::kWrite) {
      HERMES_RUN_THALLIUM->IoCallServer(req, bulk, io_type, data.data(), data_size);
      HILOG(kDebug, "(node {}) Write blob integer: {}", HERMES_RUN_CLIENT->node_id_, (int)data[0])
    }
    TaskState *exec;
    Task *orig_task;
    RpcExec(req, state_id, method, xfer, orig_task, exec);
    if (io_type == IoType::kRead) {
      HILOG(kDebug, "(node {}) Read blob integer: {}", HERMES_RUN_CLIENT->node_id_, (int)data[0])
      HERMES_RUN_THALLIUM->IoCallServer(req, bulk, io_type, data.data(), data_size);
    }
  ...
  void RpcExec(const tl::request &req,
    ...
    // Deserialize task
    exec = HERMES_RUN_TASK_REGISTRY->GetTaskState(state_id);
    if (exec == nullptr) {
      HELOG(kFatal, "(node {}) Could not find the task state {}",
            HERMES_RUN_CLIENT->node_id_, state_id);
    ...
    orig_task->domain_id_ = DomainId::GetNode(HERMES_RUN_CLIENT->node_id_);

    // Execute task
    MultiQueue *queue = HERMES_RUN_CLIENT->GetQueue(QueueId(state_id));
  ...
  void RpcComplete(const tl::request &req,
    ...
    BinaryOutputArchive<false> ar(DomainId::GetNode(HERMES_RUN_CLIENT->node_id_));
    ...
    HILOG(kDebug, "(node {}) Returning {} bytes of data (task_node={}, task_state={}/{}, method={})",
          ...
          state_id,
          method);
    HERMES_RUN_CLIENT->DelTask(exec, orig_task);
...
HERMES_RUN_TASK_CC(hrun::remote_queue::Server, "remote_queue");
