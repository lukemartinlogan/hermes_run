...
#ifndef HERMES_RUN_INCLUDE_HERMES_RUN_WORK_ORCHESTRATOR_WORKER_H_
#define HERMES_RUN_INCLUDE_HERMES_RUN_WORK_ORCHESTRATOR_WORKER_H_
 ...
 public:
  ...
  bool CheckTaskGroup(Task *task, TaskState *exec,
    ...
    if (ret == TASK_UNORDERED || task->IsUnordered()) {
      HILOG(kDebug, "(node {}) Task {} is unordered, so count remains 0 worker={}",
            HERMES_RUN_CLIENT->node_id_, task->task_node_, id_);
      return true;
    }
    ...
    if (it == group_map_.end()) {
      ...
      HILOG(kDebug, "(node {}) Increasing depth of group to {} worker={}",
            HERMES_RUN_CLIENT->node_id_, node.node_depth_, id_);
      return true;
    }
    ...
    if (node_cmp.root_ == node.root_) {
      ...
      HILOG(kDebug, "(node {}) Increasing depth of group to {} worker={}",
            HERMES_RUN_CLIENT->node_id_, node_cmp.node_depth_, id_);
      return true;
    }
    return false;
  }
  ...
  void RemoveTaskGroup(Task *task, TaskState *exec,
    ...
    HILOG(kDebug, "(node {}) Decreasing depth of to {} (task_node={} worker={})",
          HERMES_RUN_CLIENT->node_id_, node_cmp.node_depth_, task->task_node_, id_);
    if (node_cmp.node_depth_ == 0) {
  ...
  void EndTask(Lane *lane, TaskState *exec, Task *task, int &off) {
    ...
    if (exec && task->IsFireAndForget()) {
      HERMES_RUN_CLIENT->DelTask<TaskState>(exec, task);
    } else {
      task->SetComplete();
    }
  }
...
#endif  // HERMES_RUN_INCLUDE_HERMES_RUN_WORK_ORCHESTRATOR_WORKER_H_
