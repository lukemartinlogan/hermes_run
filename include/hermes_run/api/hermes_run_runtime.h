...
#ifndef HERMES_RUN_INCLUDE_HERMES_RUN_CLIENT_HERMES_RUN_SERVER_H_
#define HERMES_RUN_INCLUDE_HERMES_RUN_CLIENT_HERMES_RUN_SERVER_H_

#include "hermes_run/task_registry/task_registry.h"
#include "hermes_run/work_orchestrator/work_orchestrator.h"
#include "hermes_run/queue_manager/queue_manager_runtime.h"
#include "hermes_run_admin/hermes_run_admin.h"
#include "remote_queue/remote_queue.h"
#include "hermes_run_client.h"
#include "manager.h"
#include "hermes_run/network/rpc.h"
#include "hermes_run/network/rpc_thallium.h"

// Singleton macros
#define HERMES_RUN_RUNTIME hshm::Singleton<hrun::Runtime>::GetInstance()
#define HERMES_RUN_RUNTIME_T hrun::Runtime*
#define HERMES_RUN_REMOTE_QUEUE (&HERMES_RUN_RUNTIME->remote_queue_)
#define HERMES_RUN_THALLIUM (&HERMES_RUN_RUNTIME->thallium_)
#define HERMES_RUN_RPC (&HERMES_RUN_RUNTIME->rpc_)
...
class Runtime : public ConfigurationManager {
 ...
 private:
  ...
  void ServerInit(std::string server_config_path) {
    ...
    HERMES_RUN_CLIENT->Create(server_config_path, "", true);
    ...
    HERMES_RUN_CLIENT->MakeTaskStateId();
    admin_task = hipc::make_mptr<Admin::CreateTaskStateTask>();
    task_registry_.RegisterTaskLib("hermes_run_admin");
    task_registry_.CreateTaskState(
        "hermes_run_admin",
        "hermes_run_admin",
        HERMES_RUN_QM_CLIENT->admin_task_state_,
        admin_task.get());
    ...
    task_registry_.CreateTaskState(
        "proc_queue",
        "proc_queue",
        HERMES_RUN_QM_CLIENT->process_queue_,
        admin_task.get());

    // Create the work orchestrator queue scheduling library
    TaskStateId queue_sched_id = HERMES_RUN_CLIENT->MakeTaskStateId();
    admin_task = hipc::make_mptr<Admin::CreateTaskStateTask>();
    ...
    TaskStateId proc_sched_id = HERMES_RUN_CLIENT->MakeTaskStateId();
    admin_task = hipc::make_mptr<Admin::CreateTaskStateTask>();
    ...
    HERMES_RUN_ADMIN->SetWorkOrchQueuePolicyRoot(hrun::DomainId::GetLocal(), queue_sched_id);
    HERMES_RUN_ADMIN->SetWorkOrchProcPolicyRoot(hrun::DomainId::GetLocal(), proc_sched_id);
...
//    while (HERMES_RUN_WORK_ORCHESTRATOR->IsRuntimeAlive()) {
//      // Scheduler callbacks?
//      HERMES_THREAD_MODEL->SleepForUs(1000);
//    }
    HILOG(kInfo, "Finishing up last requests")
    HERMES_RUN_WORK_ORCHESTRATOR->Join();
...
#endif  // HERMES_RUN_INCLUDE_HERMES_RUN_CLIENT_HERMES_RUN_SERVER_H_
